...
| Scope | Both platforms (Claude Code + Droid) | Hook is added to `claude/sdd/hooks/hooks.json`, `droid/sdd/hooks/hooks.json`, and `.factory/settings.json` (local dev). |
...
- New: `droid/sdd/hooks/workflow-reminder-hook.sh` (Droid I/O wrapper)
- Edit: `claude/sdd/hooks/hooks.json` -- add `UserPromptSubmit` entry
- Edit: `droid/sdd/hooks/hooks.json` -- add `UserPromptSubmit` entry
- Edit: `.factory/settings.json` -- add `UserPromptSubmit` entry (local dev)
...
- Wired into both platform hook configs and local dev settings

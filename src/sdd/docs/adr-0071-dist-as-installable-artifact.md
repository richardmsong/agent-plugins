...
| Compiled artifacts | `docs-mcp.js` and `docs-dashboard.js` only — build.go compiles them (shells out to bun), then per-file stubs in the platform dir reference the compiled outputs via `{{ include }}`. `docs-dashboard/` is NOT a compiled artifact — it's distributed as source (see dedicated row below). | Maintains uniform stub rule: every dist/ file has a stub; compiled artifacts are no exception |
